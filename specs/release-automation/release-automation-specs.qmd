...
The structure of the files tab is shown [below](./release-automation.qmd#files-sheet). Each row in the files sheet represents a file that has to be created in the release.

The file name can be constructed using the `[name](./release-automation.qmd#name)` and `[type](./release-automation.qmd#type)` columns in the files sheet. The [`type`](./release-automation.qmd#type) column decides what the file extension should be, **.csv** for CSV files and **.xlsx** for excel files.

The [`part`](./release-automation.qmd#part) column determines where the contents of the file comes from or what to fill the file with. The column can contain an ID for a part or a set which should match up with a row in the parts sheet or sets sheet respectively. 
...
The [`addHeader`](./release-automation.qmd#addheaders) column allows the user to add a string as the first line in the file. Reasons for doing this are explained [here](https://odm.discourse.group/t/generation-of-tables-and-lists-from-the-odm-working-excel-file/99/9?u=ysequeira). Each header should be added as a cell in the first row of the sheet.
...
If the value of the `addHeader` column for it is `version;1.1.0;name;John Doe`, then the release file would be modified as below,
...
### RA-3: Deploying the files to OSF

Once the release files have been built they will need to be uploaded to their release destinations. All of this information is encoded in the [`destinations`](./release-automation.qmd#destinations) column in the [`files`](./release-automation.qmd#files-sheet) sheet.

Only the files whose `destinations` column contains the `osf` keyword should be uploaded to OSF. The [`osfLocation`](./release-automation.qmd#osflocation) column contains the path where the file should be uploaded. 

There are three states that need to handled when deplying the file to OSF,
...
### RA-4: Deploying the files to GitHub

Similar to deplying files to OSF, files whose [`destinations`](./release-automation.qmd#destinations) column contains the `github` keyword need to be uploaded to the [PHES-ODM repo](https://github.com/Big-Life-Lab/PHES-ODM). The `githubLocation` folder identifies the path where the file should be uploaded.

Similar to the OSF deployment feature, the following two states will need to be handled

1. When there are no release files on GitHub. 
    The files should be created and put in their correct locations. 
    A branch should be created and named `release-{version}` and files put in there.
...
Once the upload has been completed to all relevant destinations, a workflow should be trigged in the [PHES-ODM-Doc](https://github.com/Big-Life-Lab/PHES-ODM-Doc). This will allow the documentation repo to update itself with the new files.

### RA-6: Trigger a PR in the PHES-ODM-Validation repo

Similar to the documentation repo, a workflow will need to be trigged in the [PHES-ODM-Validation](https://github.com/Big-Life-Lab/PHES-ODM-Validation) repo to allow it to update to the new dictionary files.
...
The full list of allowed variables are documented [here](./release-automation.qmd#template-variables).
...
type: [string](./release-automation.qmd#string)

#### label

Human readable description for the file

type: [string](./release-automation.qmd#string), [nullable](./release-automation.qmd#nullable)

#### name

The name of the file in the release

type: [templateString](./release-automation.qmd#templatestring)

#### type

The file type

type: [categorical](./release-automation.qmd#categorical)
...
type: [string](./release-automation.qmd#string), [non-nullable](./release-automation.qmd#nullable)

Validations:

* The value should set to a `set` or a `part`
* The value can be sey to a `set` only if the [type](./release-automation.qmd#type) column is `excel`
...
type: [list](./release-automation.qmd#list) of [templateString](./release-automation.qmd#templatestring), [nullable](./release-automation.qmd#nullable)

#### destinations

Where the file will be uploaded to

type: [list](./release-automation.qmd#list) of [categorical](./release-automation.qmd#categorical), [non-nullable](./release-automation.qmd#nullable)

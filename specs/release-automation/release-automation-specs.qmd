...
Once the release files have been built they will need to be uploaded to their release destinations.
All of this information is encoded in the [`destinations`](./release-automation.qmd#destinations) column in the [`files`](./release-automation.qmd#files-sheet) sheet.

Files whose [`destinations`](./release-automation.qmd#destinations) column contains the `github` keyword will need to be uploaded to the [PHES-ODM repo](https://github.com/Big-Life-Lab/PHES-ODM).
The `githubLocation` folder identifies the path where the file should be uploaded.

The following two states will need to be handled

1.  When there are no release files on GitHub. The files should be created and put in their correct locations. A branch should be created and named `release-{version}` and files put in there. A commit should be made with the new files called `[BOT] release-{version}` A PR should be made from the new branch into `dev`. The PR should be called `[BOT] Release {version}`
2.  When there is a release version on GitHub 2.1. If the previous release is newer than the new release, then an error should be thrown and the entire process should stop. 2.2: Otherwise, all the old files need to be deleted. The same steps as the first state need to be followed
...
Categories \* osf \* github

Validations \* Has to have at least one destination
...
Validations: \* Required if one of the destinations is osf
...
Validations: \* Required if one of the destinations is github

# -*- coding: utf-8 -*-
"""loop_through_rules.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CDi9bIxCoJMaJnZW78WSdP4cvcncB_4L
"""
...
def loop_through_rules(rule: dict, list_of_rules: list, intermediate_filtered_data: DataFrame, \
  datetime_variables: list, numeric_variables: list, string_variables:list) -> DataFrame:
  ...
  from filter_range_by_datetime_vars import \
      filter_range_by_datetime_vars  # pylint: disable=import-error
  from filter_range_by_numeric_vars import \
      filter_range_by_numeric_vars  # pylint: disable=import-error
  ...
  for filter_by in list_of_rules:
    ...
    if rule['direction'] == 'row':
      if isinstance(filter_by, str):
        ...
        if re.search(pat3, filter_by) !=None and re.search(pat4, filter_by) != None:
          bracket = '[]'
          print('BRACKET PRESENT []')
        if re.search(pat5, filter_by) !=None and re.search(pat6, filter_by) != None:
          bracket = '()'
          print('BRACKET PRESENT ()')
        if re.search(pat5, filter_by) != None and re.search(pat4, filter_by) != None:
          bracket = '(]'
          print('BRACKET PRESENT (]')
        if re.search(pat3, filter_by) != None and re.search(pat6, filter_by) != None:
          bracket = '[)'
          print('BRACKET PRESENT [)')
      ...
      if filter_by == 'ALL':
        ...
        intermediate_filtered_data, rule_is_datetime, rule_is_numeric, \
          rule_is_char = filter_range_by_datetime_vars(bracket, \
            lower_limit, upper_limit, filter_val, intermediate_filtered_data, \
              rule_is_datetime, rule_is_numeric, rule_is_char, \
                datetime_variables, numeric_variables, string_variables)

        #filters rows of data by numeric variables
        if not rule_is_datetime:
          intermediate_filtered_data, rule_is_numeric = filter_range_by_numeric_vars(\
            bracket, lower_limit, upper_limit,\
              intermediate_filtered_data, rule_is_numeric, numeric_variables)

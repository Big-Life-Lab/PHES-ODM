# -*- coding: utf-8 -*-
"""filter_single_value.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1rJx1bM_gcUZSeEFj90gFHONQ1Yqlg1A6
"""
...
def filter_single_value(pat1: str, pat2: str, pat7: str, pat8: str, filter_val: Any, rule_is_datetime: bool, \
  rule_is_numeric: bool, rule_is_char: bool, intermediate_filtered_data: DataFrame, \
    datetime_variables: list, numeric_variables: list, string_variables: list) -> Tuple[DataFrame, bool, bool, bool]:
  ...
  if isinstance(filter_val, str):
    for datetime_var in datetime_variables:
      ...
      if (re.fullmatch(pat1, str(filter_val)) != None or re.fullmatch(pat2, str(filter_val)) !=\
         None or re.fullmatch(pat7, str(filter_val)) != None or re.fullmatch(pat8, str(filter_val))\
            != None or re.fullmatch(pat9, str(filter_val)) != None or re.fullmatch(pat10, str(filter_val)) != None):
        rule_is_datetime = True
        filter_val = pd.to_datetime(filter_val)
        intermediate_filtered_data = intermediate_filtered_data.loc[(intermediate_filtered_data[datetime_var] != filter_val),:]
      else:
    ...
    if not rule_is_datetime:
      for numeric_var in numeric_variables:
        ...
        if re.fullmatch(r'(\d+)', str(filter_val)) or re.fullmatch(r'(\d+\.\d+)', str(filter_val)):
          rule_is_numeric = True
          filter_val = eval(str(filter_val))
    ...
    for numeric_var in numeric_variables:
      if not rule_is_datetime and (isinstance(filter_val, int) or \
        isinstance(filter_val, float)):
        rule_is_numeric = True
        intermediate_filtered_data = intermediate_filtered_data.loc\
          [(intermediate_filtered_data[numeric_var] != filter_val),:]

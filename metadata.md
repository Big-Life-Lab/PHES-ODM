...
<!--# test -->There are eight tables that are described below. example data is stored in [data](data).
...
-   **measureCat**: Gene target region or other measure.
    ...
    -   `covidN1N2mean`: SARS-CoV-2 gene region average of N1 and N2
...
-   **measureUnit**: Unit of SARS-CoV-2 measurement.
    ...
    -   `PPMV`: Viral copies/copies crAssphage
...
-   **siteDescription**: Description of wastewater site that are not characterized in `siteType`, `siteSource`.
...
-   **siteSource**: Setting where the sample was taken. {add to ESR diagram}

    -   `airlane`: Airplane- `municipal`: Wastewater shed for a municipality such as city or town
    ...
    -   `submunicipal`: A municipality subdivision
    -   `other`: Other setting.

-   **siteSourceOther**: Description for other setting where sample was taken. See `primarySourceOfWastewater`. Entries can be added to the category list for `primarySourceOfWastewater`. {add to ESR diagram}
...
- **Derived, summary or transformed measures**: These three measures are generated to summarize or tranform one or more variables. An example is calculating the mean value of one or more SARS-CoV-2 regions. Normalization and standardization are other examples of a transformed measure.

The naming approach for these variables and categories follows the same approach as naming variable and category names, except use a `_` when concatenating variable or category names 

Example: mean of covidN1 and covidN2 would be describe as,
...
Mean viral copies of mean value N1 and N2 per viral copies of PMMV wide table description:
